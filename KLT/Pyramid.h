...
public:
    ...
    KLT_Pyramid(int basecols, int baserows, int subsampling, int nlevel,
            bool initMe = true);
...
public:
    ...
    void smoothAndComputePyramid(const QImage im, const Kernels* kern,
            float sigma_fact);

    int color(const float x, const float y, const int level,
            Vec3f& putHere) const;
