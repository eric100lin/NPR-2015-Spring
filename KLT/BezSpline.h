...
enum SamplingScheme
{
    RESAMPLE, REEVALUATE, RESAMPLE_CONSISTENTLY
};
...
public:
    ...
    int numCtrls() const
    {
        return _ctrlpts.size();
    }
    bool empty() const
    {
        return _ctrlpts.empty();
    }
    ...
    void clearControls();
    void addCtrl(const Vec2f& c)
    {
        _ctrlpts.push_back(c);
    }
    ...
    const std::vector<Vec2f>& getControls() const
    {
        return _ctrlpts;
    }
    std::vector<Vec2f>& getControls()
    {
        return _ctrlpts;
    }
    ...
    {
        assert(_mapLength > 0);
        _ZMap = new int[_mapLength];
    }
    void createVarMap()
    {
        assert(_mapLength > 0);
        _varMap = new int[_mapLength];
    }
    int ZMap(const int i) const
    {
        assert(i >= 0 && i < _mapLength && _ZMap);
        return _ZMap[i];
    }
    int varMap(const int i) const
    {
        assert(i >= 0 && i < _mapLength && _varMap);
        return _varMap[i];
    }
    int& ZMap(const int i)
    {
        assert(i >= 0 && i < _mapLength && _ZMap);
        return _ZMap[i];
    }
    int& varMap(const int i)
    {
        assert(i >= 0 && i < _mapLength && _varMap);
        return _varMap[i];
    }
...
private:
    ...
    bool flat(const float p, const float q, const float r,
