...
public:
    ...
    MyMontage(QImage *ims, int num, int mag, int numCols)
    {
        ...
        _mont = QImage(numCols * tw * mag, numRows * th * mag,
        ...
        for (int j = 0; j < numRows; j++)
        {
            ...
            for (int i = 0; i < numCols; i++)
            {
                ...
                int magw = ims[whichImage].width() * mag, magh =
                        ims[whichImage].height() * mag;
                for (int imj = 0; imj < magh; imj++)
                    for (int imi = 0; imi < magw; imi++)
                    {
                        assert(
                                imi / mag < ims[whichImage].width()
                                        && imj / mag
                                                < ims[whichImage].height());
