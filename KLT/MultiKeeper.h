...
public:
    ...
    MultiKeeper(const MultiTrackData* mt, int curveSampling, int hold,
            bool precond = true);
    ...
    double calculateRo(const double* p, const double newTheta,
    ...
    void take2Key0(int curve, int ib, const double* g2, double res,
            const double c);
    void take2(int b, int curve, int ib, const double* g1, const double* g2,
            double res, const double c);
    void take2Key1(int curve, int ib, const double* g1, double res,
            const double c);

    void take4Key0(int curve, int ib, const double* g2, double res,
            const double c);
    void take4(int b, int curve, int ib, const double* g1, const double* g2,
            double res, const double c);
    void take4Key1(int curve, int ib, const double* g1, double res,
            const double c);

    void take6Key0(int curve, int ib, const double* g2, const double res,
            const double c);
    void take6(int b, int curve, int ib, const double* g1, const double* g2,
            double res, const double c);
    void take6Key1(int curve, int ib, const double* g1, double res,
            const double c);

    void take4Special(int b, int curve, int ib, const double* g1, double res,
            const double c);
...
private:
    ...
    int internal_cp_to_subvar(const int curve, const int p)
    {
        ...
        return _sub_offset[keeperNum]
                + 2 * _keepers[keeperNum].subVar(keeperVar);
    }

    //void assignVarsKeepers(const int curveSampling);
    OffElems* allocateElems(bool beg, int curve, int l, int k, int Ow, int Oh);
    void processOElemsKey0(bool beg, int curve, int lcp, int kcp, int size,
            const double* g2, const double c);
    void processOElemsKey1(bool beg, int curve, int size, const double* g1,
            const double c);
    void processOElemsKeyb(bool beg, int b, int curve, int size,
            const double* g1, const double c);
    void processOElems(bool beg, int b, int curve, int size, const double* g1,
            const double* g2, const double c);
    void outerProductPlus(const double* A, const double* B, int An,
