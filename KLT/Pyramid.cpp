...
KLT_Pyramid::KLT_Pyramid(int basencols, int basenrows, int nsubsampling,
        int nnlevels, bool initMe) :
        subsampling(nsubsampling), nLevels(nnlevels), _basencols(basencols), _basenrows(
                basenrows)
{
...
void KLT_Pyramid::setupStructure(int basencols, int basenrows)
{
    ...
    for (int i = 0; i < nLevels; i++)
    {
        ...
        ncols[i] = basencols;
        nrows[i] = basenrows;
        basencols /= subsampling;
        basenrows /= subsampling;
    }
...
void KLT_Pyramid::computePyramid(KLT_FloatImage* imgIn, float sigma_fact)
{
    ...
    for (i = 1; i < nLevels; i++)
    {
        ...
        for (y = 0; y < innrows; ++y)
            for (x = 0; x < inncols; ++x)
                img[i].data[y * inncols + x] = tmpimg->data[(subsampling * y
                        + subhalf) * oldncols + (subsampling * x + subhalf)];
...
KLT_ColorPyramid::KLT_ColorPyramid(int basecols, int baserows, int subsampling,
        int nlevel)
{
...
void KLT_ColorPyramid::smoothAndComputePyramid(const QImage im,
        const Kernels* kern, float sigma_fact)
{
...
int KLT_ColorPyramid::color(const float x, const float y, const int level,
        Vec3f& putHere) const
{
    if (x < 0 || y < 0)
        return -1;
    ...
    float *ptr_r = _r->getFImage(level)->data + offset, *ptr_g = _g->getFImage(
            level)->data + offset, *ptr_b = _b->getFImage(level)->data + offset;
...
void KLT_ColorPyramid::writeImages(char* name) const
{
    ...
    for (int t = 0; t < nlevels; t++)
    {
        ...
        KLT_FloatImage *rf = _r->getFImage(t), *gf = _g->getFImage(t), *bf =
                _b->getFImage(t);
        for (int j = 0; j < h; j++)
            for (int i = 0; i < w; i++)
            {
                int offset = j * w + i;
                im.setPixel(i, j,
                        qRgb(BRACK(rf->g(offset)), BRACK(gf->g(offset)),
                                BRACK(bf->g(offset))));
            }
