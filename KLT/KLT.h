...
#include "kltSpline.h"
    ...
    void setupLongCurveTrack2(const KLT_FullCPyramid** pyrms,
    ...
    int longCurveTrack(const KLT_FullCPyramid** pyrms, const int numFrames,
            const int numP, const int wwidth, const int wlh, const int whh,
            Vec2f* Plocs, Vec2f* Z);
    int longCurveReshape(int numFrames, int numP, Vec2f* Plocs, Vec2f* Z,
            Vec2f* Sb, bool pL);
    ...
    const KLT_FullCPyramid **__pyrms;
    const KLT_FullPyramid **__pyrmsE;
    MultiTrackData* _mt;
    bool __redo;
...
private:
    ...
    void longCurveTrack2(const KLT_FullCPyramid** pyrms,
    ...
    void initEdgeMins(const KLT_FullPyramid** pyrmsE, const int level,
            const double invsubs);
    double createMatrices2(const KLT_FullCPyramid** pyrms, const int numFrames,
            const int l, const int wwidth, const int wlh, const int whh,
            const Vec2f* Plocs, Vec2f* Z, const int level, const double invsubs,
            Keeper* keep);
    double createMatrices3(const KLT_FullCPyramid** pyrms,
    ...
    void calculateNewK(double* K, const double i, const double j,
            const double invsubs) const;
    void hnew(const int n, const float i, const float j, const float invc,
            const Vec2f* P, Vec2f& result) const;
    void hnew2(const int cp, const int c, const int t, const float i,
            const float j, const float invsubs, const Vec2f* Z,
            Vec2f& result) const;
    int steihaugSolver(GenKeeper* im, double* x, const double trustRadius,
            bool* boundaryHit, double* error2ptr = NULL); // returns # iterations
    double createTestSolution(const Vec2f* Z, const double* sol,
            const int numVec, const int l, Vec2f* Z2);
    void projectToTR(double *x, const double* d, const double* p,
            double trustRadius, int n);
    void projectToTR2(double *x, const double* d, const double* p,
            double trustRadius, int n);
    bool internalLongCurveTrack2(const KLT_FullCPyramid** pyrms,
            const KLT_FullPyramid** pyrmsE, const int level,
            const double invsubs, const int curveSampling, const int hold,
            const int maxIterations);
    void safeTrack2(const KLT_FullCPyramid** pyrms,
            const KLT_FullPyramid** pyrmsE, const int level,
            const double invsubs, const int curveSampling, const int hold,
            const int maxIterations);
    ...
    void transform(const float x, const float y, float* xOut, float* yOut,
            const float* Z);
    void transform(const float x, const float y, float* xOut, float* yOut,
            const double* Z);
    void computeIntensityDifference(KLT_FloatImage* img1, KLT_FloatImage* img2,
            const int window_width, const int window_height, float x1, float y1,
            const float* Z, float* imgdiff, int* big_status);
    void computeGradientSum(KLT_FloatImage* gradx1, KLT_FloatImage* grady1,
            KLT_FloatImage* gradx2, KLT_FloatImage* grady2,
            const int window_width, const int window_height, float x1, float y1,
            float* Z, float* gradx, float* grady, int* big_status);
    void compute6by6GradientMatrix(float* gradx, float* grady,
            const int window_width, const int window_height, double T[36]);
    void compute6by1ErrorVector(float* imgdiff, float* gradx, float* grady,
    ...
    int internalColorTrackFeature(float x1, float y1, double* Z,
            ...
            const KLT_FullCPyramid *aPyrm, const KLT_FullCPyramid *bPyrm,
            const int level);
    //int internalLongPatchTrack2(const KLT_FullCPyramid** pyrms, int numFrames, float ax, float ay,
    //		      int window_width, int window_height,
    //		      double* param, const int level);

    void addnbyk(double* Z, const int span, const double coeff,
            const double* adder, const int n, const int k);
    void addnbyk(double* Z, const int span, const double* adder, const int n,
            const int k);
    ...
    int calculateMtNorm(const int n, const int i, const int j, const int l,
            const Vec2f* P, double *G, double *KS, double *MS);
    void calculateK(double* K, const int n, const double i, const double j,
            const double c, const int l) const;
    void calculateKStrip(double* K, const double i, const double j,
            const double invc) const;
    void calculateKStrip0(double* K, const double i, const double j,
            const double invc) const;
    void calculateKStripl(double* K, const double i, const double j,
            const double invc) const;
    void calculateKStripNorm(double* K, const int n, const double i,
            const double j, const Vec2f* P) const;
    void calculateKStripNorm0(double* K, const double i, const double j,
            const Vec2f* P) const;
    void calculateKStripNorml(double* K, const int n, const double i,
            const double j, const Vec2f* P) const;
    void h(const int n, const float i, const float j, const int l,
            const float c, const Vec2f* P, Vec2f& result) const;
    void hnorm(const int n, const float i, const float j, const int l,
            const Vec2f* P, Vec2f& result) const;
    void rmmultplus(double *rm, double *a, double *b, int n, int m,
            int l) const;
    void rmmult(double *mat, double *a, double *b, int m, int k, int n) const;
    int internalLongCurveTrack(const KLT_FullCPyramid** pyrms,
    ...
    int internalSetulbTrack(const KLT_FullCPyramid** pyrms, const int numFrames,
            const int l, const int wwidth, const int wlh, const int whh,
            const Vec2f* Plocs, Vec2f* Z, const int level, const float spacing);
    double evaluateObj(double* objJac, const KLT_FullCPyramid** pyrms,
            const int numFrames, const int l, const int wwidth, const int wlh,
            const int whh, const Vec2f* Plocs, const double* Zd,
...
inline void KLT_TrackingContext::rmmult(double *rm, double *a, double *b, int n,
        int m, int l) const
{
    double z, *q0, *p, *q;
    int i, j, k;
...
inline void KLT_TrackingContext::rmmultplus(double *rm, double *a, double *b,
...
inline void KLT_TrackingContext::addnbyk(double* Z, const int span,
        const double coeff, const double* adder, const int n, const int k)
{
    int i, j, shop, khop;
