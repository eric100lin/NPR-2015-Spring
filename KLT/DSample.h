...
public:
    ...
    DSample(const Vec2f& loc, const Vec2f& tan, const Vec2f& curv,
            const float baset, const float t) :
    ...
    DSample(const DSample& o)
    {
        ...
        _tangent = o._tangent;
        _baset = o._baset;
        _curvature = o._curvature;
        _t = o._t;
    }

    bool operator<(const DSample& o) const
    {
        return (_baset + _t) < (o._baset + o._t);
    }
    bool operator<(const double ot) const
    {
        return (_baset + _t) < ot;
    }
    bool operator<(const float ot) const
    {
        return (_baset + _t) < ot;
    }
