...
void KLT_TrackingContext::setupSplineTrack(const KLT_FullCPyramid** pyrms,
        const KLT_FullPyramid** pyrmsE, MultiSplineData* mts, bool redo)
{
...
#define DELETE_STK
void KLT_TrackingContext::splineTrack(const KLT_FullCPyramid** pyrms,
...
void KLT_TrackingContext::safeSplineTrack(const KLT_FullCPyramid** pyrms,
...
bool KLT_TrackingContext::innerSplineTrack(const KLT_FullCPyramid** pyrms,
        const KLT_FullPyramid** pyrmsE, const int level)
{
    int iteration = 0;
    ...
    CSplineKeeper *keep1 = new CSplineKeeper(_mts), *keep2 = new CSplineKeeper(
            _mts);
    ...
    currTheta = createSplineMatrices(pyrms, pyrmsE, &(_mts->_Z), level,
            keep1 /*, imgKeep, edgeKeep*/);
    ...
    do
    {
        ...
        while (maxStep > .1 && trustRadius >= prec && iteration <= maxIterations)
        {
            ...
            if (_stateOk)
                newTheta = createSplineMatrices(pyrms, pyrmsE, &Z2, level,
...
double KLT_TrackingContext::createSplineMatrices(const KLT_FullCPyramid** pyrms,
        const KLT_FullPyramid** pyrmsE, ZVec* Z, const int level,
        CSplineKeeper* keep
        ...
        )
{
    ...
    int vars00[4], *vars01 = NULL, vars02[4], vars10[4], *vars11 = NULL,
            vars12[4];
    ...
    int imgCompCount = 0, edgeCompCount = 0, smooth0Count = 0, smooth1Count = 0,
            smooth2Count = 0;
    ...
    /*if (imgKeep)
     ...
     edgeKeep->refresh();*/
    //printf("Here 1\n");
    CSplineKeeper *imgKeep = new CSplineKeeper(keep), *edgeKeep =
            new CSplineKeeper(keep), *D0Keep = new CSplineKeeper(keep),
            *D1Keep = new CSplineKeeper(keep); // D2Keep is just keep, which is already refreshed

    ObsCache imgCache(imgKeep), D0Cache(D0Keep), D1Cache(D1Keep), D2Cache(keep),
            ECache(edgeKeep);
    ...
    for (c = 0; c < _mts->_nCurves && _stateOk; c++) // iterate over curves
    {
        ...
        { //_mts->discretize(t, c, pow2(level));
            ...
            assert(
                    !useEdges || !_mts->useEdges(c)
                            || _mts->_edgeMins[c].size() == un);
            for (n = 0; n <= un; ++n) // iterate over samples of curve
            {
                ...
                {
                    if (n != un)
                        _mts->getDiscreteSample(t, c, n + 1, &ds02, vars02);
                    vars01 = vars;
                }
                else if (n != un)
                    _mts->getDiscreteSample(t, c, n + 1, &ds02);
                TrackSample& ds01 = ds;
                ...
                if (useImage && n % stride == 0)
                {
                    ...
                    for (j = _mts->_trackWidths[c].x();
                            j <= _mts->_trackWidths[c].y(); ++j)
                    {
                        ...
                        if (validPoint == 0)
                        {
...

                            {
                                ...
                                if (t < ul)
                                {
                                    calculateSplineK(ds1, ds1._k, j, invsubs,
                                            K1); // K is 2x8
                                    for (di = 0; di < 16; ++di)
                                        K1[di] *= -1.;
                                ...
                                if (t > 0)
                                    checkOk = checkOk
                                            & imgCache.checkVars(vars, 0);
                                if (t < ul)
                                    checkOk = checkOk
                                            & imgCache.checkVars(vars1, 1);

                                if (checkOk)
                                {
                                    imgCache.newObs();
                                    if (t > 0)
                                        imgCache.takeJac(grad, 0);
                                    if (t < ul)
                                        imgCache.takeJac(grad1, 1);
                                    imgCache.outer_prod(col.r());
                                    imgCache.newObs();
                                    if (t > 0)
                                        imgCache.takeJac(grad + 8, 0);
                                    if (t < ul)
                                        imgCache.takeJac(grad1 + 8, 1);
                                    imgCache.outer_prod(col.g());
                                    imgCache.newObs();
                                    if (t > 0)
                                        imgCache.takeJac(grad + 16, 0);
                                    if (t < ul)
                                        imgCache.takeJac(grad1 + 16, 1);
                                ...
                                else
                                {
                                    ...
                                    if (t == 0)
                                    {
                                        imgKeep->take8Key(vars1, grad1, col.r(),
                                                col.g(), col.b());
                                    ...
                                    {
                                        imgKeep->take8Key(vars, grad, col.r(),
                                                col.g(), col.b());
                                    ...
                                    else
                                    {
                                        ...
                                        imgKeep->take88(vars, vars1, grad,
                                                grad1, col.r());
                                        imgKeep->take88(vars, vars1, grad + 8,
                                                grad1 + 8, col.g());
                                        imgKeep->take88(vars, vars1, grad + 16,
                                                grad1 + 16, col.b());
                                    }
                                } // end cache miss
                ...
                // EDGE TERM
                if (useEdges && n % stride == 0 && n != un && t > 0 && t < ul
                        && _mts->useEdges(c) && _mts->_edgeMins[c][n] > 0)
                {

                    splineLoc(ds, 0, invsubs, &loc);
                    double eval = pyrmsE[t]->img->getFImage(level)->interpolate(
                            loc.x(), loc.y(), &validPoint);
                    G_t[0] = pyrmsE[t]->gradx->getFImage(level)->interpolate(
                            loc.x(), loc.y(), &di);
                    G_t[1] = pyrmsE[t]->grady->getFImage(level)->interpolate(
                            loc.x(), loc.y(), &di);
                    ...
                    if (validPoint == 1)
                    {
                        ...
                        if (checkOk)
                        {
                            ...
                            ECache.outer_prod(eval,
                                    1. / (edgeMins[n] * edgeMins[n]));
                        }
                        else
                        {
                            ECache.recordMiss(1);
                            imgKeep->take8(vars, grad, eval,
                                    1. / (edgeMins[n] * edgeMins[n]));
                        }
                ...
                if (_useD0)
                {
                    ...
                    if (checkOk)
                    {
                        D0Cache.newObs();
                        if (t > 0)
                            D0Cache.takeJac(ds01, 0, 1., 0);
                        if (t < ul)
                            D0Cache.takeJac(ds11, 1, -1., 0);
                        D0Cache.outer_prod(delta.x());
                        D0Cache.newObs();
                        if (t > 0)
                            D0Cache.takeJac(ds01, 0, 0, 1.);
                        if (t < ul)
                            D0Cache.takeJac(ds11, 1, 0, -1.);
                ...
                if (_useD1)
                {
                    ...
                    {
                        if (n == un || ds1num == un1)
                            continue;  // can't do last samples
                        ...
                        checkOk = true;
                        if (t > 0)
                            checkOk = checkOk & D1Cache.checkVars(vars01, 0)
                                    & D1Cache.checkVars(vars02, 0);
                        if (t < ul)
                            checkOk = checkOk & D1Cache.checkVars(vars11, 1)
                                    & D1Cache.checkVars(vars12, 1);
                        ...
                        if (checkOk)
                        {
                            ...
                            if (t > 0)
                            {
                                D1Cache.takeJac(ds01, 0, 2 * delta1.x(),
                                        2 * delta1.y());
                                D1Cache.takeJac(ds02, 0, -2 * delta1.x(),
                                        -2 * delta1.y());
                            }
                            if (t < ul)
                            {
                                D1Cache.takeJac(ds11, 1, -2 * delta2.x(),
                                        -2 * delta2.y());
                                D1Cache.takeJac(ds12, 1, 2 * delta2.x(),
                                        2 * delta2.y());
                            }
                        ...
                        else
                        {
                            ...
                            if (t > 0)
                            {
                                putVarsOnJ(J, ds01, vars01, 2 * delta1.x(),
                                        2 * delta1.y());
                                putVarsOnJ(J, ds02, vars02, -2 * delta1.x(),
                                        -2 * delta1.y());

                            }
                            if (t < ul)
                            {
                                putVarsOnJ(J, ds11, vars11, -2 * delta2.x(),
                                        -2 * delta2.y());
                                putVarsOnJ(J, ds12, vars12, 2 * delta2.x(),
                                        2 * delta2.y());
                            }
                ...
                if (_useD2)
                {
                    ...
                    {
                        if (n == 0 || n == un || ds1num == 0 || ds1num == un1)
                            continue;     // no sample terms for endpoints
                        ...
                        if (t < ul)
                            ...
                            _mts->getExistingCorrDiscreteSample(t, c, n - 1,
                                    &ds10, vars10);
                        else
                        ...
                        checkOk = true;
                        if (t > 0)
                            checkOk = checkOk & D2Cache.checkVars(vars00, 0)
                                    & D2Cache.checkVars(vars01, 0)
                                    & D2Cache.checkVars(vars02, 0);
                        if (t < ul)
                            checkOk = checkOk & D2Cache.checkVars(vars10, 1)
                                    & D2Cache.checkVars(vars11, 1)
                                    & D2Cache.checkVars(vars12, 1);
    ...
    for (i = 0; i < numFrames; ++i)
        thetaSum += theta[i] * invcw + thetas1[i] * invd1 + thetas[i] * invd2
                + thetas0[i] * invd0 + thetaE[i] * invew;
...
void KLT_TrackingContext::putGradOnJ(Ubcv& J1, Ubcv& J2, Ubcv& J3,
        const double grad[24], const int vars[4])
{
    if (tt6 == 0)
        time6.start();
    else
...
void KLT_TrackingContext::putVarsOnJ(Ubcv& J1, Ubcv& J2, const TrackSample& ds,
        const int vars[4], const double coef)
{
    if (tt3 == 0)
        time3.start();
    else
...
void KLT_TrackingContext::putVarsOnJ(Ubcv& J, const TrackSample& ds,
        const int vars[4], const double c1, const double c2)
{
    if (tt3 == 0)
        time3.start();
    else
...
void KLT_TrackingContext::noSubSplineLoc(const TrackSample& ds, int j,
        float subs, Vec2f* loc)
{
    *loc = ds._loc;
...
// Given a TrackSample with loc & normal, calculate location j off
void KLT_TrackingContext::splineLoc(const TrackSample& ds, int j, float invsubs,
        Vec2f* loc)
{
    *loc = ds._loc;
    *loc *= invsubs; // downsample before adding tangent term
...
void KLT_TrackingContext::initSplineEdgeMins(const KLT_FullPyramid** pyrmsE,
        const int level)
{
