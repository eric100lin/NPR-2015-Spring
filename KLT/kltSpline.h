...
void safeSplineTrack(const KLT_FullCPyramid** pyrms,
        const KLT_FullPyramid** pyrmsE, const int level);

bool innerSplineTrack(const KLT_FullCPyramid** pyrms,
        const KLT_FullPyramid** pyrmsE, const int level);

double createSplineMatrices(const KLT_FullCPyramid** pyrms,
...
void putGradOnJ(Ubcv& J1, Ubcv& J2, Ubcv& J3, const double grad[24],
        const int vars[4]);
void putVarsOnJ(Ubcv& J1, Ubcv& J2, const TrackSample& ds, const int vars[4],
        const double coef);
void putVarsOnJ(Ubcv& J, const TrackSample& ds, const int vars[4],
...
void calculateSplineK(const TrackSample& s, double k, int j, double invsubs,
        double K[16]);
...
QTime time1, time2, time3, time4, time5, time6, time7, time8, time9, time10,
        time11;
