...
// HERE CHANGE
double MultiSplineData::createTestSol(const ZVec* Z, const double* sol,
        ZVec* Z2) const
{
    assert(Z != Z2);
    ...
    for (c = 0; c < _nCurves; ++c)
    {
        for (t = 1; t < _numFrames; ++t) // don't do keyframes
        {
            ...
            for (n = 0; n < numC; ++n)
            {
                ...
                maxStep = MAX(MAX(maxStep, fabs(sol[varIndex])),
                        fabs(sol[varIndex + 1]));
...
void MultiSplineData::discretizeAll(int interval, SamplingScheme ss,
        bool onlyInbetweens)
{
    int i = 0, j, c, t;
    for (c = 0; c < _nCurves; ++c)
    {
        ...
        {
            if ((t == 0 || t == _numFrames) && onlyInbetweens)
                continue;
...
void MultiSplineData::getDiscreteSample(int t, int c, int n, TrackSample* res,
        int vars[4]) const
{
...
void MultiSplineData::getDiscreteSample(int t, int c, int n,
        TrackSample* res) const
{
...
void MultiSplineData::getCorrDiscreteSample(int t, int c, int n,
        TrackSample *res, int vars[4]) const
{
...
int MultiSplineData::getExistingCorrDiscreteSample(int t, int c, int n,
        TrackSample *res, int vars[4]) const
{
    assert(t >= 0 && t < _numFrames - 1); // no variables last frame
    assert(c >= 0 && c < _nCurves);
    //if (t != _numFrames-1) {        // should be identity
    getDiscreteSample(t + 1, c, n, res, vars);
    return n;
    //}
        ...
        TrackSample *res) const
{
    ...
    if (t != _numFrames - 1)    // should be identity
    {
        getDiscreteSample(t + 1, c, n, res);
        return n;
    }

    getCorrDiscreteSample(t, c, n, res);
    return -1; // G!
    ...
    SampleHolder::const_iterator found = std::lower_bound(latersh.begin(),
            latersh.end(), ot);
...
void MultiSplineData::getSample(float tp, int t, int c, TrackSample* res,
        int vars[4]) const
{
    const BezSpline* bez = getSpline(c, t);
...
{
    const BezSpline *last = getSpline(c, _numFrames - 1), *first = getSpline(c,
            1);
...
std::vector<double>* MultiSplineData::refreshEdgeMins(const int c,
        const int num)
{
    //if (_edgeMins[c] != NULL)
    //delete[] _edgeMins[c];
...
TrackSample::TrackSample(const DSample& o) :
        DSample(o)
{
