...
SplineKeeper::SplineKeeper(MultiSplineData* mts) :
        _mts(mts)
{
    ...
    if (precond())
    {
        ...
        for (FixedControlV::const_iterator j = flocs.begin(); j != flocs.end();
                ++j)
        {
        ...
        _sweeper = new HB_Sweep(mts->_nCurves, numvars, mts->_numFrames - 1,
                fixers);
        delete[] fixers;
        delete[] numvars;
...
void SplineKeeper::constructMultiDiagMatrix()
{
    ...
    maxWidths.reserve(numCurves);
    dims.reserve(numCurves);
    ...
    for (uint i = 0; i < (uint) numCurves; ++i)
    {
        ...
        if (_mts->_numFrames != 2)
            maxWidths.push_back(
                    2 * (_mts->tcn_var(2, i, 1) - _mts->tcn_var(1, i, 1)) + 14);
        else
...
double SplineKeeper::calculateRo(const double* p, const double newTheta,
...
void SplineKeeper::take8Key(int vars[4], double* J, double r1, double r2,
        double r3)
{
    int i, j;
    ...
    for (i = 0, iv = 0; i < 8; i += 2, ++iv)
    {
        ...
        mat.takeUnrdF(vars[iv], vars[iv],
                J[i] * J[i] + J[8 + i] * J[8 + i] + J[16 + i] * J[16 + i]);
        mat.takeUnrdF(vars[iv] + 1, vars[iv] + 1,
                J[i + 1] * J[i + 1] + J[9 + i] * J[9 + i]
                        + J[17 + i] * J[17 + i]);
...
// (J = concat J1, J2);
void SplineKeeper::take88(int vars1[4], int vars2[4], double* J1, double* J2,
        double r)
{
    int i, j;

    //_crapgDirty = true;
    memcpy(_vars, vars1, 4 * sizeof(int));
    memcpy(_vars + 4, vars2, 4 * sizeof(int));
