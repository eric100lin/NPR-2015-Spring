...
ContCorr::ContCorr(const int numSegs, const int numSamples) :
...
ContCorr::ContCorr(FILE *fp)
{
    int res;

    res = fread(&_identity, sizeof(bool), 1, fp);
    assert(res == 1);
    if (!_identity)
    {
        res = fread(&_n, sizeof(int), 1, fp);
        _samples.reserve(_n);
        res = fread(_samples.data(), sizeof(float), _n, fp);
        _domain.reserve(_n);
        res = fread(_domain.data(), sizeof(float), _n, fp);
    }
    else
        _n = -1;

    _numSamples = -1;
    res = fread(&_aTop, sizeof(float), 1, fp);
    assert(res == 1);
    res = fread(&_bTop, sizeof(float), 1, fp);
    assert(res == 1);
}

void ContCorr::save(FILE* fp)
{
    fwrite(&_identity, sizeof(bool), 1, fp);
    if (!_identity)
    {
        assert(_samples.size() > 0 && _domain.size() > 0);
        fwrite(&_n, sizeof(int), 1, fp);
        fwrite(_samples.data(), sizeof(float), _n, fp);
        fwrite(_domain.data(), sizeof(float), _n, fp);
    }
    fwrite(&_aTop, sizeof(float), 1, fp);
    fwrite(&_bTop, sizeof(float), 1, fp);
}
...
float ContCorr::getT(const float t) const
{
    ...
    {
        // calculate slope
        float m = (_samples[index + 1] - _samples[index])
                / (_domain[index + 1] - _domain[index]);
