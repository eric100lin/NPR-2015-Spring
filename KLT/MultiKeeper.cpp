...
MultiKeeper::MultiKeeper(const MultiTrackData* mt, int curveSampling, int hold,
        bool preconde) :
        _mt(mt), _precond(preconde)
{
    ...
    {
        _keepers[c].init(mt->_numFrames - 1, mt->_crs[c]._nVars * 2,
                curveSampling, hold);
    ...
    if (precond())
    {
        ...
        for (i = 0; i < _mt->_nCurves; ++i)
        {
            ...
            for (j = 0; j < flocs.size(); ++j) //SPEED, not good
            {
                ...
                index +=
                        flocs[j]._frame
                                * internal_cp_to_keeper(i, flocs[j]._i)->numVarOneBlock();
                assert(index + 1 < _subsNvar * 2);
                fixers[i].push_back(index);
            }
            numvars[i] = _keepers[i].numVarOneBlock() / 2;
        }
        _sweeper = new HB_Sweep(mt->_nCurves, numvars, mt->_numFrames - 1,
                fixers);
        delete[] fixers;
        delete[] numvars;
    }
...
double MultiKeeper::calculateRo(const double* p, const double newTheta,
        double const oldTheta) const
{
    int nv = _subsNvar * 2;
...
double MultiKeeper::createTestSol(const Vec2f* Z, const double* sol,
        Vec2f* Z2) const
{
    int zup = 0;
...
void MultiKeeper::take2Key0(int curve, int ib, const double* g2, double res,
        const double c)
{
    ...
    internal_cp_to_keeper(curve, ib)->take2Key0(
            internal_cp_to_keeper_var(curve, ib), g2, res, c);
    //debug_jac(0,curve,ib,2,g2,res,c);
}

void MultiKeeper::take2(int b, int curve, int ib, const double* g1,
        const double* g2, double res, const double c)
{
    memset(_G1, 0, 6 * sizeof(double));
    ...
    internal_cp_to_keeper(curve, ib)->take2(b,
            internal_cp_to_keeper_var(curve, ib), g1, g2, res, c);
    //debug_jac(b,curve,ib,2,g1,g2,res,c);
}

void MultiKeeper::take2Key1(int curve, int ib, const double* g1, double res,
        const double c)
{
    memset(_G1, 0, 6 * sizeof(double));
    //int nib, maxj;
    ...
    internal_cp_to_keeper(curve, ib)->take2Key1(
            internal_cp_to_keeper_var(curve, ib), g1, res, c);
    //debug_jac(_mt->_numFrames-2,curve,ib,2,g1,res,c);
}

void MultiKeeper::take4Key0(int curve, int ib, const double* g2, double res,
        const double c)
{
    memset(_G1, 0, 6 * sizeof(double));
    if (ib == 0 && !_mt->begWithRest(curve))
    {
        ...
        convert1(1, 1, ib, curve, nib1, nib2, maxj1, maxj2, g2);
        assert(maxj1 == 2);
    ...
    else if (ib == _mt->_crs[curve]._nPoints - 2 && !_mt->endWithRest(curve))
    {
        ...
        internal_cp_to_keeper(curve, ib + 1)->_takejKey0(maxj2, nib2,
                _G1 + maxj1, res, c);
...
void MultiKeeper::take4(int b, int curve, int ib, const double* g1,
        const double* g2, double res, const double c)
{
    memset(_G1, 0, 6 * sizeof(double));
    memset(_G2, 0, 6 * sizeof(double));
    ...
    {
        int nib1, nib2, maxj1, maxj2;
        convert2(1, 1, ib, curve, nib1, nib2, maxj1, maxj2, g1, g2);
        assert(maxj1 == 2);
        _keepers[curve]._takej(maxj2, b, nib2, _G1 + 2, _G2 + 2, res, c);
        internal_cp_to_keeper(curve, 0)->_takej(maxj1, b, nib1, _G1, _G2, res,
                c);
    ...
    else if (ib == _mt->_crs[curve]._nPoints - 2 && !_mt->endWithRest(curve))
    {
        ...
        internal_cp_to_keeper(curve, ib + 1)->_takej(maxj2, b, nib2,
                _G1 + maxj1, _G2 + maxj1, res, c);
...
void MultiKeeper::take4Key1(int curve, int ib, const double* g1, double res,
        const double c)
{
    memset(_G1, 0, 6 * sizeof(double));
    if (ib == 0 && !_mt->begWithRest(curve))
    ...
    else if (ib == _mt->_crs[curve]._nPoints - 2 && !_mt->endWithRest(curve))
    {
        ...
        internal_cp_to_keeper(curve, ib + 1)->_takejKey1(maxj2, nib2,
                _G1 + maxj1, res, c);
...
void MultiKeeper::take6Key0(int curve, int ib, const double* g2,
        const double res, const double c)
{
    memset(_G1, 0, 6 * sizeof(double));
    if (ib == 0 && !_mt->begWithRest(curve))
    {
        int nib1, nib2, maxj1, maxj2;
        convert1(1, 2, ib, curve, nib1, nib2, maxj1, maxj2, g2);
        assert(maxj1 == 2);
        _keepers[curve]._takejKey0(maxj2, nib2, _G1 + 2, res, c);
        internal_cp_to_keeper(curve, 0)->_takejKey0(maxj1, nib1, _G1, res, c);
        processOElemsKey0(true, curve, 1, 0, maxj1 + maxj2, _G1, c);
    ...
    else if (ib == _mt->_crs[curve]._nPoints - 3 && !_mt->endWithRest(curve))
    {
        ...
        _keepers[curve]._takejKey0(maxj1, nib1, _G1, res, c);
        internal_cp_to_keeper(curve, ib + 2)->_takejKey0(maxj2, nib2,
                _G1 + maxj1, res, c);
...
void MultiKeeper::take6(int b, int curve, int ib, const double* g1,
        const double* g2, double res, const double c)
{
    memset(_G1, 0, 6 * sizeof(double));
    memset(_G2, 0, 6 * sizeof(double));
    ...
    {
        int nib1, nib2, maxj1, maxj2;
        convert2(1, 2, ib, curve, nib1, nib2, maxj1, maxj2, g1, g2);
        assert(maxj1 == 2);
        _keepers[curve]._takej(maxj2, b, nib2, _G1 + 2, _G2 + 2, res, c);
        internal_cp_to_keeper(curve, 0)->_takej(maxj1, b, nib1, _G1, _G2, res,
                c);
    ...
    else if (ib == _mt->_crs[curve]._nPoints - 3 && !_mt->endWithRest(curve))
    {
        ...
        _keepers[curve]._takej(maxj1, b, nib1, _G1, _G2, res, c);
        internal_cp_to_keeper(curve, ib + 2)->_takej(maxj2, b, nib2,
                _G1 + maxj1, _G2 + maxj1, res, c);
        processOElems(false, b, curve, maxj1 + maxj2, _G1, _G2, c);
    }
    else
...
void MultiKeeper::take6Key1(int curve, int ib, const double* g1, double res,
        const double c)
{
    memset(_G1, 0, 6 * sizeof(double));
    if (ib == 0 && !_mt->begWithRest(curve))
    {
        int nib1, nib2, maxj1, maxj2;
        convert1(1, 2, ib, curve, nib1, nib2, maxj1, maxj2, g1);
        assert(maxj1 == 2);
        _keepers[curve]._takejKey1(maxj2, nib2, _G1 + 2, res, c);
        internal_cp_to_keeper(curve, 0)->_takejKey1(maxj1, nib1, _G1, res, c);
        processOElemsKey1(true, curve, maxj1 + maxj2, _G1, c);
    ...
    else if (ib == _mt->_crs[curve]._nPoints - 3 && !_mt->endWithRest(curve))
    {
        ...
        internal_cp_to_keeper(curve, ib + 2)->_takejKey1(maxj2, nib2,
                _G1 + maxj1, res, c);
        processOElemsKey1(false, curve, maxj1 + maxj2, _G1, c);
    }
    else
...
void MultiKeeper::take4Special(int b, int curve, int ib, const double* g1,
        double res, const double c)
{
    ...
    if (ib == 0 && !_mt->begWithRest(curve))
    {
        ...
        internal_cp_to_keeper(curve, 0)->_takejKeyb(b, maxj1, nib1, _G1, res,
                c);
    ...
    else if (ib == _mt->_crs[curve]._nPoints - 2 && !_mt->endWithRest(curve))
    {
        ...
        internal_cp_to_keeper(curve, ib + 1)->_takejKeyb(b, maxj2, nib2,
                _G1 + maxj1, res, c);
...
OffElems* MultiKeeper::allocateElems(bool beg, int curve, int l, int k, int Ow,
        int Oh)
{
    OffElems* oe = NULL;
    if (beg)
...
void MultiKeeper::processOElemsKey0(bool beg, int curve, int lcp, int kcp,
        int size, const double* g2, const double c)
{
...
void MultiKeeper::processOElemsKey1(bool beg, int curve, int size,
        const double* g1, const double c)
{
    OffElems* oe = NULL;
    if (beg)
        oe = _b_oelems[curve];
...
void MultiKeeper::processOElemsKeyb(bool beg, int b, int curve, int size,
        const double* g1, const double c)
{
    OffElems* oe = NULL;
...
// assume off elements already created
void MultiKeeper::processOElems(bool beg, int b, int curve, int size,
        const double* g1, const double* g2, const double c)
{
    OffElems* oe = NULL;
    if (beg)
        oe = _b_oelems[curve];
...
// result is An x 2
void MultiKeeper::outerProductPlus(const double* A, const double* B, int An,
        double* putHere, const double co)
{
...
void MultiKeeper::convert2(int anum, int bnum, int ib, int curve, int& nib1,
        int& nib2, int& maxj1, int& maxj2, const double* g1, const double* g2)
{
    int lib = internal_cp_to_keeper_var(curve, ib);
    maxj1 = internal_cp_to_keeper(curve, ib)->convert2(lib, anum, nib1, g1, g2,
            _G1, _G2);
    assert(maxj1 <= anum * 2);
    lib = internal_cp_to_keeper_var(curve, ib + anum);
    maxj2 = internal_cp_to_keeper(curve, ib + anum)->convert2(lib, bnum, nib2,
            g1 + 2 * anum, g2 + 2 * anum, _G1 + maxj1, _G2 + maxj1);
}

void MultiKeeper::convert1(int anum, int bnum, int ib, int curve, int& nib1,
        int& nib2, int& maxj1, int& maxj2, const double* g1)
{
    int lib = internal_cp_to_keeper_var(curve, ib);
    maxj1 = internal_cp_to_keeper(curve, ib)->convert1(lib, anum, nib1, g1,
            _G1);
    assert(maxj1 <= anum * 2);
    lib = internal_cp_to_keeper_var(curve, ib + anum);
    maxj2 = internal_cp_to_keeper(curve, ib + anum)->convert1(lib, bnum, nib2,
            g1 + 2 * anum, _G1 + maxj1);
}
...
void MultiKeeper::printSubs(FILE* fp)
{
    int w = 0, h = 0;
    for (int c = 0; c < _mt->_nCurves; ++c)
        for (int t = 0; t < _mt->_numFrames - 1;
                ++t, h += _keepers[c].numVarOneBlock(), w +=
                        _keepers[c].origNVOneBlock())
