...
double Keeper::calculateRo(const double* p, const double newTheta,
        const double oldTheta) const
{
    ...
    for (b = 0; b < _num_blocks; ++b, sol_off += _block_size) // iterate over blocks
    {
        for (v = 0; v < obs2; ++v, ++Z_off) // iterate over variables in a block
        {
            ...
            if (_subs[v]._k != 1.)
            {
                ...
                if ((tmp = MAX(fabs(sol[sol_i + 2]), fabs(sol[sol_i + 3])))
                        > maxStep)
                {
                    maxStep = tmp;
                    assert(maxStep < 1000);
...
void Keeper::printSubs2(FILE* fp, int w, int h)
{
    ...
    for (r = 0; r < onv; r++)
    {
        fprintf(fp, "%d %d %.10f\n", w + 2 * r + 1, h + 2 * _subs[r]._P + 1,
                _subs[r]._k);
        fprintf(fp, "%d %d %.10f\n", w + 2 * r + 2, h + 2 * _subs[r]._P + 2,
                _subs[r]._k);
        if (_subs[r]._k != 1.)
        {
            fprintf(fp, "%d %d %.10f\n", w + 2 * r + 1, h + 2 * _subs[r]._P + 3,
                    1. - _subs[r]._k);
            fprintf(fp, "%d %d %.10f\n", w + 2 * r + 2, h + 2 * _subs[r]._P + 4,
                    1. - _subs[r]._k);
        }
    }
}
...
void Keeper::take2(int b, int ib, const double* g1, const double* g2,
...
void Keeper::take4(int b, int ib, const double* g1, const double* g2,
        double res, const double c)
{
    memset(_G1, 0, 4 * sizeof(double));
    memset(_G2, 0, 4 * sizeof(double));
...
void Keeper::take6Key0(int ib, const double* g2, const double res,
        const double c)
{
...
void Keeper::take6(int b, int ib, const double* g1, const double* g2,
        double res, const double c)
{
    memset(_G1, 0, 6 * sizeof(double));
...
void Keeper::take6Key1(const int ib, const double* g1, double res,
        const double c)
{
    memset(_G1, 0, 6 * sizeof(double));
    int nib, maxj;
...
int Keeper::convert2(int ib, const int hm, int& nib, const double* g1,
        const double* g2)
{
...
int Keeper::convert2(int ib, const int hm, int& nib, const double* g1,
        const double* g2, double* G1, double* G2)
{
    ib /= 2;
...
void Keeper::_take2Key0(const int ib, const double* g2, double res,
        const double c)
{
...
void Keeper::_take4Key0(const int ib, const double* g2, double res,
        const double c)
{
...
void Keeper::_take6Key0(const int ib, const double* g2, double res,
        const double c)
{
...
void Keeper::_take2Key1(const int ib, const double* g1, double res,
        const double c)
{
    assert(ib <= _block_size - 2);
...
void Keeper::_take4Key1(const int ib, const double* g1, double res,
        const double c)
{
    assert(ib <= _block_size - 4);
    int row = (_num_blocks - 1) * _block_size + ib, i;
...
void Keeper::_take6Key1(const int ib, const double* g1, double res,
        const double c)
{
    assert(ib <= _block_size - 6);
    int row = (_num_blocks - 1) * _block_size + ib, i;
...
void Keeper::_take2Keyb(const int b, const int ib, const double* g1, double res,
        const double c)
{
    assert(ib <= _block_size - 2);
...
void Keeper::_take4Keyb(const int b, const int ib, const double* g1, double res,
        const double c)
{
    assert(ib <= _block_size - 4);
    int row = b * _block_size + ib, i;
...
void Keeper::_take2(const int b, const int ib, const double* g1,
...
void Keeper::_take4(const int b, const int ib, const double* g1,
        const double* g2, double res, const double c)
{
    assert(b < _num_blocks - 1);
    assert(ib <= _block_size - 4);
...
void Keeper::_take6(const int b, const int ib, const double* g1,
        const double* g2, double res, const double c)
{
    assert(b < _num_blocks - 1);
    assert(ib <= _block_size - 6);
...
void Keeper::_takejKey0(int j, const int ib, const double* g2, double res,
        const double c)
{
    if (j == 2)
...
void Keeper::_takejKey1(int j, const int ib, const double* g1, double res,
        const double c)
{
    if (j == 2)
...
void Keeper::_takej(int j, const int b, const int ib, const double* g1,
        const double* g2, double res, const double c)
{
    if (j == 2)
...
void Keeper::_takejKeyb(const int b, int j, const int ib, const double* g1,
        double res, const double c)
{
    if (j == 2)
