...
public:
    ...
    void discretizeAll(int interval, SamplingScheme ss, bool onlyInbetweens =
            false);

    // returns number of samples at time t, curve c
    int getNumSamples(int t, int c) const;

    void getDiscreteSample(int t, int c, int n, TrackSample* res,
            int vars[4]) const;
    void getDiscreteSample(int t, int c, int n, TrackSample* res) const;

    // Also, get corresponding sample one frame later
    // creates new sample
    void getCorrDiscreteSample(int t, int c, int n, TrackSample *res,
            int vars[4]) const;
    ...
    int getExistingCorrDiscreteSample(int t, int c, int n, TrackSample *res,
            int vars[4]) const;
    int getExistingCorrDiscreteSample(int t, int c, int n,
    ...
    int tc_numControls(const int t, const int c) const
    {
        return tc_numSegs(t, c) * 3 + 1;
    }
    ...
    const FixedControlV& getFixedLocs() const
    {
        return _fixedControls;
    }
    ...
    void addMask(unsigned char* mask)
    {
        _masks.push_back(mask);
    }
    void takeMasks(MultiSplineData* o);
    void setMaskDims(const int w, const int h)
    {
        _maskw = w;
        _maskh = h;
    }
