...
template<class T, int initSize> inline DynArray<T, initSize>::DynArray(
...
template<class T, int initSize> inline DynArray<T, initSize>& DynArray<T,
        initSize>::operator=(const DynArray<T, initSize>& other)
{
    delete[] data;
    space = other.count;
    if (space == 0)
