...
namespace FitCurves
{
    extern "C"
    {
        ...
        {
            AbstractPath::_talkFit.addBez(curve[0].x, curve[0].y, curve[1].x,
                    curve[1].y, curve[2].x, curve[2].y, curve[3].x, curve[3].y);
        }
    }
}
...
RotoPath::RotoPath(const RotoPath& other) :
        AbstractPath(other)
{
    mutualInit();
    _lh = other._lh;
...
void RotoPath::finishPrevCorrespondence()
{
    if (!_prevFrame)
        return;
    assert(
            _prevFrame && _prevFrame->_nextFrame == this
                    && !_prevFrame->_nextCont);

    assert(!_samplingDirty && !_prevFrame->_samplingDirty);
    assert(
            _prevFrame->getNumElements()
                    == _prevFrame->_bez->getDiscreteCount());
...
void RotoPath::render(bool showTrackPoints, bool visEffort) const
{
    ...
    if (visEffort && _bez && !_bez->empty()
            && int(_touched.size()) == _bez->numCtrls())
    {
...
void RotoPath::hnew(const int n, const float i, const float j, const Vec2f* P,
        Vec2f& result) const
{
    Vec2f Tn(P[n + 1], P[n]);
    Tn *= .5f; //invc;
...
void RotoPath::h(const int n, const float i, const float j, const int l,
        const float invc, const Vec2f* P, Vec2f& result) const
{
    Vec2f Tn, Nn;
    assert(n < l);
    if (n == l - 1)
...
{
    if (!(_nextFrame && _nextCont && !_samplingDirty))
        return;
...
void RotoPath::rotateSelectedAbout(float dtheta, const Vec2f center)
{
    ...
    Vec2f a(costheta, -sintheta), b(sintheta, costheta);
...
{
    return (find(_internalsFixed.begin(), _internalsFixed.end(), which)
            != _internalsFixed.end());
}
...
//void RotoPath::save(FILE* fp, int version)
//{
//    int num_points = getNumElements();
//    fwrite(&num_points, sizeof(int), 1, fp); // write # of points
//    fwrite(getData(), sizeof(Vec2f), num_points, fp);

//    if (_tangents)
//    {
//        fwrite(&num_points, sizeof(int), 1, fp);
//        fwrite(_tangents, sizeof(Vec2f), num_points, fp);
//    }
//    else
//    {
//        int dummy = 0;
...
//    int num_CorrDraws = getNumCorrDraws();
//    fwrite(&num_CorrDraws, sizeof(int), 1, fp); // write # of corresponding drawPaths
//    for (set<DrawPath*>::const_iterator c = _corrDraws.begin();
//            c != _corrDraws.end(); ++c)
//        (*c)->writePtr(fp);
...
//    fwrite(&_fixed, sizeof(bool), 1, fp);

//    fwrite(&_trackEdges, sizeof(bool), 1, fp); // wether to track edges

//    int bJointsSize = _bJoints.size();
//    fwrite(&bJointsSize, sizeof(int), 1, fp); // # beginning joints
//    for (int i = 0; i < bJointsSize; ++i)
//    {
//        _bJoints[i]._rp->writePtr(fp);
//        fwrite(&(_bJoints[i]._side), sizeof(int), 1, fp);
//    }

//    int eJointsSize = _eJoints.size();
//    fwrite(&eJointsSize, sizeof(int), 1, fp); // # end joints
//    for (int i = 0; i < eJointsSize; ++i)
//    {
...
//    else
//    {
//        int num_bezSeg = _bez->numSegs();
//        fwrite(&num_bezSeg, sizeof(int), 1, fp);
//        int num_ctrls = _bez->numCtrls();
//        fwrite(_bez->getControls().data(), sizeof(Vec2f), num_ctrls, fp);
//        fwrite(&_samplingDirty, sizeof(bool), 1, fp);
//    }

//    bool has_prevCont = (_prevCont != NULL);
//    fwrite(&has_prevCont, sizeof(bool), 1, fp);
//    if (has_prevCont)
//        _prevCont->save(fp);

//    bool has_nextCont = (_nextCont != NULL);
//    fwrite(&has_nextCont, sizeof(bool), 1, fp);
//    if (has_nextCont)
//        _nextCont->save(fp);

//    int internalsFixedSize = _internalsFixed.size();
//    fwrite(&internalsFixedSize, sizeof(int), 1, fp);
...
//    int touchedSize = _touched.size();
//    fwrite(&touchedSize, sizeof(int), 1, fp);
//    for (std::vector<bool>::const_iterator c = _touched.begin();
//            c != _touched.end(); ++c)
//    {
//        bool bValue = *c;
//        fwrite(&bValue, sizeof(bool), 1, fp);
//    }

//    fwrite(&_trimapWidth, sizeof(int), 1, fp);
//}
...
//    // read # of points
//    int num_points;
//    res = fread(&num_points, sizeof(int), 1, fp);
//    assert(res == 1);
//    addNElements(num_points);

//    // read points
//    res = fread(getData(), sizeof(Vec2f), num_points, fp);
//    assert(res == num_points);

//    // read _tangent status
//    int has_tangents;
//    res = fread(&has_tangents, sizeof(int), 1, fp);
//    assert(res == 1);
//    if (has_tangents != 0)
//    {
//        assert(has_tangents = getNumElements());
//        _tangents = new Vec2f[getNumElements()];
//        res = fread(_tangents, sizeof(Vec2f), num_points, fp);
//        assert(res == num_points);
//    }
//    else
//        _tangents = NULL;

//    // read # of corresponding drawPaths
//    int num_CorrDraws;
//    res = fread(&num_CorrDraws, sizeof(int), 1, fp);
//    assert(res == 1);
//    for (int i = 0; i < num_CorrDraws; i++)
//    {
//        int dptr[2];
...
//    res = fread(&_lh, sizeof(int), 1, fp);
//    assert(res == 1);
//    res = fread(&_hh, sizeof(int), 1, fp);
//    assert(res == 1);
//    res = fread(&_fixed, sizeof(bool), 1, fp);
//    assert(res == 1);

//    res = fread(&_trackEdges, sizeof(bool), 1, fp);
//    assert(res == 1);

//    int bJointsSize;
//    res = fread(&bJointsSize, sizeof(int), 1, fp);
//    assert(res == 1);
//    assert(bJointsSize >= 0 && bJointsSize < 100);
//    for (int i = 0; i < bJointsSize; i++)
//    {
//        Joint j;
//        res = fread(dptr, sizeof(int), 2, fp);
...
//        res = fread(&(j._side), sizeof(int), 1, fp);
//        assert(res == 1);
//        _bJoints.push_back(j);
//    }

//    int eJointsSize;
//    res = fread(&eJointsSize, sizeof(int), 1, fp);
//    assert(res == 1);
//    assert(eJointsSize >= 0 && eJointsSize < 100);
//    for (int i = 0; i < eJointsSize; ++i)
//    {
//        Joint j;
//        res = fread(dptr, sizeof(int), 2, fp);
...
//    res = fread(_endsFixed, sizeof(bool), 2, fp);
//    assert(res == 2);

//    int has_bez;
//    res = fread(&has_bez, sizeof(int), 1, fp);
//    assert(res == 1);
//    if (has_bez == -1)
//        _bez = NULL;
//    else
//    {
//        _bez = new BezSpline();
//        _bez->setNumSegs(has_bez);
//        int num_ctrls = has_bez * 3 + 1;
//        _bez->getControls().reserve(num_ctrls);
//        res = fread(_bez->getControls().data(), sizeof(Vec2f), num_ctrls, fp);
//        assert(res == num_ctrls);
//        res = fread(&_samplingDirty, sizeof(bool), 1, fp);
//        assert(res == 1);
//    }

//    bool has_prevCont;
//    res = fread(&has_prevCont, sizeof(bool), 1, fp);
//    assert(res == 1);
//    if (has_prevCont)
//    {
//        _prevCont = new ContCorr(fp);
//    }
//    else
//        _prevCont = NULL;

//    bool has_nextCont;
//    res = fread(&has_nextCont, sizeof(bool), 1, fp);
//    assert(res == 1);
//    if (has_nextCont)
//    {
//        _nextCont = new ContCorr(fp);
//    }
//    else
//        _nextCont = NULL;

//    int internalsFixedSize;
//    res = fread(&internalsFixedSize, sizeof(int), 1, fp);
//    assert(res == 1);
//    for (int i = 0; i < internalsFixedSize; ++i)
//    {
//        int set_data;
//        res = fread(&set_data, sizeof(int), 1, fp);
//        assert(res == 1);
//        _internalsFixed.insert(set_data);
//    }

//    int touchedSize;
//    res = fread(&touchedSize, sizeof(int), 1, fp);
//    assert(res == 1);
//    for (int i = 0; i < touchedSize; ++i)
//    {
//        bool bValue;
//        res = fread(&bValue, sizeof(bool), 1, fp);
//        assert(res == 1);
//        _touched.push_back(bValue);
//    }

//    res = fread(&_trimapWidth, sizeof(int), 1, fp);
//    assert(res == 1);
//}
...
void RotoPath::insertJoint(const Joint& j, int w)
{
    ...
    assert (jointsOk(_bJoints));assert
    (jointsOk(_eJoints));}
...
{
    Joints::iterator c;
    assert (jointsOk(_bJoints));for
(	c = _bJoints.begin(); c != _bJoints.end(); ++c)
...
{
    Joints::iterator c;
    assert (jointsOk(_eJoints));for
(	c = _eJoints.begin(); c != _eJoints.end(); ++c)
    c->_rp->removeJoint(this, c->_side);
    _eJoints.clear();
}
...
void RotoPath::destroyAllCoincidentJoints(const int s)
{
    ...
    assert (allJointsOk());Joints& jnt = joints(s);
    Joints::iterator c;
...
{
    assert (jointsOk(_bJoints));assert(jointsOk(_eJoints));
    if (side==0)
    {
...
void RotoPath::copyBackJoint(Joints* js, const PathV* subtree, int side)
{
    for (Joints::iterator c = js->begin(); c != js->end(); ++c)
        if (c->_rp->prevC())
        {
            ...
            if (find(subtree->begin(), subtree->end(), c->_rp)
                    == subtree->end())
            {
                Joint j2;
                j2._rp = _prevFrame;
...
void RotoPath::setTouchedAll(const bool val)
{
    ...
    for (std::vector<bool>::iterator c = _touched.begin(); c != _touched.end();
            ++c)
        (*c) = val;
}
...
void RotoPath::estimateTouched()
{
    ...
    if (endFixed(0))
        setCtrlTouched(0, true);
    if (endFixed(1))
        setCtrlTouched(_bez->numCtrls() - 1, true);
...
void RotoPath::buildBackReconcileJoints(PathV& paths, int bFrame, int aFrame)
{
    ...
    {
        for (pc = currPrevPaths.begin(), j = 0; pc != currPrevPaths.end();
                ++pc, ++j)
        {
...
void RotoPath::buildForwardReconcileJoints(PathV& paths, int aFrame, int bFrame)
{
    ...
    {
        for (pc = currNextPaths.begin(), j = 0; pc != currNextPaths.end();
                ++pc, ++j)
        {
...
bool RotoPath::touched(const int i) const
{
    ...
    if (i == 0)
        return (_touched[i] || jointTouched(0));
    if (i == _touched.size() - 1)
        return (_touched[i] || jointTouched(1));
    return (_touched[i]);
}
...
bool RotoPath::endTouched(const int side) const
{
    if (side == 0)
        ...
        return _touched[_touched.size() - 1];
}
...
SplineSampleIterator& SplineSampleIterator::operator++()
{
    ...
    if (startedOk && int((*_samples)[_i]._baset) != *_j)
        ++_j;

    // if not in selected, keep going
    while (int((*_samples)[_i]._baset) != *_j && _i < (int) _samples->size())
        ++_i;
