...
//    for (set<DrawPath*>::const_iterator c = corrDraws->begin();
//            c != corrDraws->end(); ++c)
//        (*c)->vacateRotoCorr();
    ...
    for (short s = 0; s < 2; ++s)
    {
        if (rp->getRegion(s))
        {
            std::deque<RotoRegion*>::iterator c = std::remove(_regions.begin(),
                    _regions.end(), rp->getRegion(s)), c2;
            c2 = c;
...
void RotoCurves::setupJointsOneSide(RotoPath* rp, int side)
{
    ...
    double tmp;
    RotoPathList::iterator c;
    unsigned _paths_size = _paths.size();
    for (c = _paths.begin(); c != _paths.end(); ++c)
    {
        RotoPath *c_paths = *c;
        if (c_paths == rp)
            continue;  // CORRECTNESS: allow looping curves
        if ((tmp = c_paths->distanceToSide2(rp->getEnd(side), 0)) < _J_DIST_)
        {
            printf("Joining %d side to beginning of other curve\n", side);
            makeJoint(rp, c_paths, side, 0);
        }
        if ((tmp = c_paths->distanceToSide2(rp->getEnd(side), 1)) < _J_DIST_)
        {
            printf("Joining %d side to end of other curve\n", side);
            makeJoint(rp, c_paths, side, 1);
        }
        assert(c_paths->okEnds());
    }
}
...
void RotoCurves::setupEndJoints(RotoPath* rp)
{
    ...
    for (c = _paths.begin(); c != _paths.end(); ++c)
    {
        ...
        if ((*c) == rp)
            continue;  // CORRECTNESS: allow looping curves
        if ((*c)->distanceToStart(
                rp->getElement(rp->getNumElements() - 1)) < _J_DIST_)
            makeJoint(rp, (*c), 1, 0);
        if ((*c)->distanceToEnd(
                rp->getElement(rp->getNumElements() - 1)) < _J_DIST_)
            makeJoint(rp, (*c), 1, 1);
