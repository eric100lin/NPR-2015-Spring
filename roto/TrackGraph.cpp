...
void TrackGraph::processConstraint(const RotoPath* constrained,
        const RotoPath* constrainee, const int edside, const int eeside)
{
...
void TrackGraph::traceVarLoc(PathV::const_iterator pc, JointRecord* jr,
        int pcside, const int side)
{
...
void TrackGraph::buildMulti(MultiSplineData* mts)
{
    ...
    for (i = 0; i < mts->_nCurves; ++i)
    {
        ...
        mts->_trackWidths[i].Set(_key0paths[i]->lowHeight(),
                _key0paths[i]->highHeight());
        ...
        for (j = 0; j <= mts->_numFrames; ++j)
        {
            ...
            printf("curve %d time %d curve %x, ends %d %d, fixers %x %x\n", i,
                    j, curr, curr->endFixed(0), curr->endFixed(1), jr.fixer[0],
                    jr.fixer[1]);

            // First control
            if (j > 0 && j < mts->_numFrames
                    && (jr.fixer[0] != NULL || curr->endFixed(0))) // constraining curve, or nudged
            {
                FixedControl fcp;
                fcp._t = j;
                fcp._n = 0;
            ...
            else
            {
                ...
                if (j > 0 && j < mts->_numFrames)
                    spline->varMap(0) = mts->end_tc_var(j, which,
                            _joints[i].side[0]);
            }
            ...
            // Last Control
            if (j > 0 && j < mts->_numFrames
                    && (jr.fixer[1] != NULL || curr->endFixed(1))) // constraining curve, or nudged
            {
                FixedControl fcp;
                fcp._t = j;
                fcp._n = numControls - 1;
                fcp._c = i;
                mts->_fixedControls.push_back(fcp);
            }
            ...
            {
                spline->ZMap(numControls - 1) = mts->end_tc(j, which,
                        _joints[i].side[1]);
                if (j > 0 && j < mts->_numFrames)
                    spline->varMap(numControls - 1) = mts->end_tc_var(j, which,
                            _joints[i].side[1]);
            }
