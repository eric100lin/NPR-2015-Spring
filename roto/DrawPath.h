...
public:
    ...
    const DrawContCorr* getCorrRoto() const
    {
        return _corrRoto;
    }
    ...
    static void fillBiInterpolatedCurve(DrawPath* D0, DrawPath* D1,
    ...
    void resample(const int* numSamples)
    {
        AbstractPath::resample(numSamples, &_thick);
    }

    void fixLoc();

    const AbstractPath* getShouldBe()
    {
        return _shouldBe;
    }
    void repToShouldBe();

    void takeT1D1(int* a, int n)
    {
        _T1_D1 = a;
        _T1_D1_n = n;
    }
