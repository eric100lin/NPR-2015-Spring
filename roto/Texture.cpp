...
const int Texture::Texture_Widths[NUM_TEXTURES] =
{ 1024, 1024, 1024, 1024, 2048, 2048 };
const int Texture::Texture_Heights[NUM_TEXTURES] =
{ 1024, 1024, 1024, 16, 64, 64 };

// in case we don't want to consider the full width of the stroke texture
const float Texture::Texture_Radii[NUM_TEXTURES] =
{ 1.0, 1.0, 1.0, 0.85, 0.50, 0.50 };

const Texture_Type Texture::Texture_Types[NUM_TEXTURES] =
{ paper, paper, paper, stroke, stroke, stroke };
char* Texture::Texture_FNames[NUM_TEXTURES] =
{ "textures/charcoal_cropped.png", "textures/pastel_cropped.png",
        "textures/pencil_cropped.png", "textures/pencil_stroke_1024_16.png",
...
Texture::Texture(fileType ftype, GLuint name, char* fileName, int width,
        int height)
{
...
void Texture::TextureBMP(GLuint name, char* fileName, int width, int height)
{
    ...
    for (int row = 0; row < height; row++)
    {
        ...
        for (int col = 0; col < width; col++)
        {
            ...
            unsigned char luminance = 0.299
                    * dataRGB[3 * (row * width + col) + 0]
    ...
    for (int row = 0; row < height; row++)
    {
        ...
        for (int col = 0; col < width; col++)
        {
            ...
            unsigned char luminance = 0.299 * qRed(color)
                    + 0.587 * qGreen(color) + 0.114 * qBlue(color);
            if (luminance < 50)
                luminance = 0;
