...
public:
    ...
    {
        assert(_bez);
        return _bez->getLoc(t);
    }

    Vec2f getTangent(float t) const
    {
        assert(_bez);
        return _bez->getTangent(t);
    }

    int getNumControls() const
    {
        assert(_bez);
        return _bez->numCtrls();
    } // /2 space
    ...
    const Vec2f* getCtrl(const int i)
    {
        assert(_bez);
        return _bez->getCtrl(i);
    }
    const Vec2f& getCtrlRef(const int i)
    {
        assert(_bez);
        return *(_bez->getCtrl(i));
    }
    const BezSpline* getBez() const
    {
        return _bez;
    }
    ...
    {
        assert(_bez);
        return _bez->isCtrlEnd(i, whichEnd);
    }
    const Vec2f& getEnd(const int which) const
    {
        assert(_bez->numCtrls() > 0);
        return _bez->getEnd(which);
    }
    ...
    {
        assert(_bez);
        return _bez->distanceToEnd2(pt, side);
    }
    double distanceToEnds2(const Vec2f pt)
    {
        assert(_bez);
        return MIN(_bez->distanceToEnd2(pt, 0), _bez->distanceToEnd2(pt, 1));
    }
    double distanceToInternalEnds2(const Vec2f pt, int* whichCtrl)
    {
        assert(_bez);
        return _bez->distanceToInternalEnds2(pt, whichCtrl);
    }
    double distanceToCtrls2(const Vec2f pt, int* whichCtrl)
    {
        assert(_bez);
        return _bez->distanceToCtrls2(pt, whichCtrl);
    }
    ...
    double distanceToEnd(const Vec2f pt) const
    {
        assert(0);
        return 0;
    }
    double distanceToStart(const Vec2f pt) const
    {
        assert(0);
        return 0;
    }
    ...
    {
        assert(_bez);
        return _bez->distToSamples2(loc, sample);
    }
    ...
    {
        Vec2f p(x, y);
        if (count == 0 || p != getElement(count - 1))
            add(p);
    }
    ...
    const std::set<DrawPath*>* getCorrDraws() const
    {
        return &_corrDraws;
    }

    std::set<DrawPath*>* getCorrDraws()
    {
        return &_corrDraws;
    }

    int getNumCorrDraws() const
    {
        return _corrDraws.size();
    }
    void emptyCorrDraws()
    {
        _corrDraws.clear();
    ...
    void clearTransformSelected()
    {
        _transformSelected.clear();
    }
...
//    void load(FILE* fp, int version);
    ...
    bool nextCorrs()
    {
        return (_nextCont != NULL && _nextFrame != NULL);
    }
    void takeNextCont(RotoPath* o)
    {
        _nextCont = o->_nextCont;
        o->_nextCont = NULL;
    }
    ...
    int highHeight() const
    {
        return _hh;
    }
    void setLowHeight(const int h)
    {
        _lh = h;
    }
    void setHighHeight(const int h)
    {
        _hh = h;
    }
    ...
    bool samplingDirty() const
    {
        return _samplingDirty;
    }

    void setFixed(const bool b)
    {
        _fixed = b;
    }
    bool fixed()
    ...
    bool trackEdges() const
    {
        return _trackEdges;
    }
    void flipTrackEdges()
    {
        _trackEdges = !_trackEdges;
    }
    void setTrackEdges(const bool i)
    {
        _trackEdges = i;
    }
    ...
    const Joints* bJoints() const
    {
        return &_bJoints;
    }
    const Joints* eJoints() const
    {
        return &_eJoints;
    }
    Joints* bJoints()
    ...
    bool endFixed(const int which) const
    {
        assert(which == 0 || which == 1);
        return _endsFixed[which];
    }
    bool anyEndFixed() const
    {
        return _endsFixed[0] || _endsFixed[1];
    }
    bool internalFixed(const int which) const;
    bool somehowFixed(const int which) const;
    void unfixCtrl(const int which);

    ContCorr* getNextCont()
    {
        return _nextCont;
    }

    static void copySetToVector(std::vector<RotoPath*>* v, std::set<RotoPath*>* s);
    ...
    static void buildForwardReconcileJoints(PathV& paths, int aFrame,
            int bFrame);

    void setRegion(RotoRegion *r, const short side)
    {
        _regions[side] = r;
    }
    RotoRegion* getRegion(const short side)
    {
        return _regions[side];
    }
    void forgetRegion(const RotoRegion* r);

    void setTrimapWidth(const int i)
    {
        _trimapWidth = i;
    }
...
private:
    ...
    void hnew(const int n, const float i, const float j, const Vec2f* P,
            Vec2f& result) const;
    ...
    mutable std::set<DrawPath*> _corrDraws;
