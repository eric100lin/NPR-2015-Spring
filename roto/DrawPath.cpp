...
GLuint DrawPath::_texture_names[NUM_TEXTURES] =
{ 0, 0, 0, 0, 0, 0 };
...
DrawPath::DrawPath(const DrawPath& other) :
        AbstractPath(other)
{
    //_corrs = NULL;
    //_corrsPtr = NULL;
...
void DrawPath::vacateRotoCorr()
{
    ...
    for (set<RotoPath*>::const_iterator c = rotoset.begin(); c != rotoset.end();
            ++c)
...
void fuckCombine(GLdouble coords[3], GLdouble* vd[4], GLfloat weight[4],
        void **outData)
{
...
void DrawPath::calculateFillDisplayList()
{
    ...
    (tobj, GLU_TESS_ERROR, (GLvoid (*)()) &fuck);gluTessCallback
...
bool DrawPath::correspondRoto2(RotoCurves* rc, const int w, const int h)
{
    ...
    int numRotos = 0;
    for (set<RotoPath*>::const_iterator c = rotos.begin(); c != rotos.end();
            ++c)
    {
        (*c)->addCorrDraw(this);
        ++numRotos;
...
// STUB
void DrawPath::fillBiInterpolatedCurve(DrawPath* D0, DrawPath* D1,
        DrawPath *Dnew, double t, int *D0_T0_T1_i, RotoPath** D0_T0_T1_p,
...
void DrawPath::setCurrTexture(int i)
{
    ...
    if (DrawPath::_texture_names[i] == 0)
    {
        ...
        else
        {
            ...
            Texture* canvas = new Texture(PNG, new_texture,
                    Texture::Texture_FNames[i], Texture::Texture_Widths[i],
                    Texture::Texture_Heights[i]);
            delete canvas;
        }
