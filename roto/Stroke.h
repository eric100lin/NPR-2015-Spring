...
inline StrokeNumT makeStrokeNum(long int num, int)
{
    return num;
}
struct _Vertex;
...
private:
    ...
    void drawCap(const _CapData & cap, vector<Point> & inner_points,
    ...
    void subdivideFourPoint(vector<PointR> * inputCurve,
            vector<PointR> * outputCurve);
    void subdivide(vector<PointR> * inputCurve, vector<PointR> * outputCurve);
    ...
    void computeCapPoints(PointR p0, float dx, float dy, float* cap_offsets,
            bool orientation);
    static void blendArray(float arrOut[], float arrIn[], int sizeOut,
            int sizeIn, int startIndex);
...
public:
    ...
    Stroke(const vector<PointR> & control, float radius, GLuint color,
            StrokeNumT n);
    ...
    vector<PointR> & control()
    {
        return _control;
    }
    const vector<PointR> & control() const
    {
        return _control;
    }
    StrokeNumT num() const
    {
        return _num;
    }
    ...
    GLubyte red() const
    {
        return getRed(_color);
    }
    GLubyte green() const
    {
        return getGreen(_color);
    }
    GLubyte blue() const
    {
        return getBlue(_color);
    }
    GLubyte alpha() const
    {
        return getAlpha(_color);
    }
    GLuint color() const
    {
        return _color;
    ...
    GLuint adjustedColor() const
    {
        return _adjustedColor;
    }
    GLuint & adjustedColor()
    {
        return _adjustedColor;
    }
